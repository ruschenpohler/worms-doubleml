...
**Control specification does not drive findings.** The null effects in ATE and BLP coefficients are not dependent on the composition of the adjustment set, which we confirm by excluding the noisy parental education proxy entirely: all coefficients are essentially unchanged.
